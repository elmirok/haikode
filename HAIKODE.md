...
The older `AI_NETWORK=1` make flag is accepted as a compatibility alias when
`HAIKODE_AI_NETWORK` is not set.
Before the first Haiku build, run `make doctor`; if dependencies are missing,
run `make install-deps`, then `make doctor` again before `make`.
