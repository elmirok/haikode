		...
		case B_OBSERVER_NOTICE_CHANGE:
		{
			...
			switch (code) {
				...
				case kMsgProjectSettingsUpdated:
				{
					...
					if (project == nullptr) {
						LogInfo("ProjectBrowser: Update project configuration message "
							"without a project folder pointer (not an error if logged once on load).");
						if (Logger::IsDebugEnabled()) {
							message->PrintToStream();
						}
						break;
...
// TODO: Cleanup
struct {
	bool operator()(ProjectItem* A, ProjectItem* B) const
	{
		return BPrivate::NaturalCompare(A->Text(), B->Text()) < 0;
	}
