...
public:
	...
	void SetDefaultSettings(const char* theme)
	{
		...
		SetCursorColor(handler->getRGB(PREF_CURSOR_FORE_COLOR),
			handler->getRGB(PREF_CURSOR_BACK_COLOR));
		SetSelectColor(handler->getRGB(PREF_SELECT_FORE_COLOR),
		...
		for (uint i = 0; i < kANSIColorCount; i++) {
			SetTermColor(i, handler->getRGB(keys[i]), false);
		}
...
GenioTermView::Instantiate(BMessage* data)
{
	if (validate_instantiation(data, "GenioTermView")) {
		WrapTermView *view = new (std::nothrow) WrapTermView(data);
		...
		if (theme.IsEmpty())
			theme = gPreferred.name;
		view->SetDefaultSettings(theme.String());
