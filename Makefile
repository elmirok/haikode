...
ifeq ($(DEBUGGER),$(filter $(DEBUGGER),TRUE 1))
	NAME := Genio_debug
	# for ASSERT()
...
SRCS += src/config/psp/BMessagePSP.cpp
SRCS += src/config/psp/AttributePSP.cpp
SRCS += src/config/psp/YamlPSP.cpp
SRCS += src/config/psp/NoStorePSP.cpp
...
SRCS += src/editor/EditorManager.cpp
